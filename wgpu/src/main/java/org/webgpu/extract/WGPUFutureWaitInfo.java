// Generated by jextract
package org.webgpu.extract;

import java.lang.invoke.*;
import java.lang.foreign.*;
import java.nio.ByteOrder;
import java.util.*;
import java.util.function.*;
import java.util.stream.*;

import static java.lang.foreign.ValueLayout.*;
import static java.lang.foreign.MemoryLayout.PathElement.*;

/**
 * {@snippet lang = c :
 * struct WGPUFutureWaitInfo {
 *     WGPUFuture future;
 *     WGPUBool completed;
 * }
 * }
 */
public class WGPUFutureWaitInfo {

    WGPUFutureWaitInfo() {
        // Should not be called directly
    }

    private static final GroupLayout $LAYOUT = MemoryLayout.structLayout(
            WGPUFuture.layout().withName("future"),
            webgpu_h.C_INT.withName("completed"),
            MemoryLayout.paddingLayout(4)).withName("WGPUFutureWaitInfo");

    /**
     * The layout of this struct
     */
    public static final GroupLayout layout() {
        return $LAYOUT;
    }

    private static final GroupLayout future$LAYOUT = (GroupLayout) $LAYOUT.select(groupElement("future"));

    /**
     * Layout for field:
     * {@snippet lang = c : * WGPUFuture future
     * }
     */
    public static final GroupLayout future$layout() {
        return future$LAYOUT;
    }

    private static final long future$OFFSET = 0;

    /**
     * Offset for field:
     * {@snippet lang = c : * WGPUFuture future
     * }
     */
    public static final long future$offset() {
        return future$OFFSET;
    }

    /**
     * Getter for field:
     * {@snippet lang = c : * WGPUFuture future
     * }
     */
    public static MemorySegment future(MemorySegment struct) {
        return struct.asSlice(future$OFFSET, future$LAYOUT.byteSize());
    }

    /**
     * Setter for field:
     * {@snippet lang = c : * WGPUFuture future
     * }
     */
    public static void future(MemorySegment struct, MemorySegment fieldValue) {
        MemorySegment.copy(fieldValue, 0L, struct, future$OFFSET, future$LAYOUT.byteSize());
    }

    private static final OfInt completed$LAYOUT = (OfInt) $LAYOUT.select(groupElement("completed"));

    /**
     * Layout for field:
     * {@snippet lang = c : * WGPUBool completed
     * }
     */
    public static final OfInt completed$layout() {
        return completed$LAYOUT;
    }

    private static final long completed$OFFSET = 8;

    /**
     * Offset for field:
     * {@snippet lang = c : * WGPUBool completed
     * }
     */
    public static final long completed$offset() {
        return completed$OFFSET;
    }

    /**
     * Getter for field:
     * {@snippet lang = c : * WGPUBool completed
     * }
     */
    public static int completed(MemorySegment struct) {
        return struct.get(completed$LAYOUT, completed$OFFSET);
    }

    /**
     * Setter for field:
     * {@snippet lang = c : * WGPUBool completed
     * }
     */
    public static void completed(MemorySegment struct, int fieldValue) {
        struct.set(completed$LAYOUT, completed$OFFSET, fieldValue);
    }

    /**
     * Obtains a slice of {@code arrayParam} which selects the array element at
     * {@code index}.
     * The returned segment has address
     * {@code arrayParam.address() + index * layout().byteSize()}
     */
    public static MemorySegment asSlice(MemorySegment array, long index) {
        return array.asSlice(layout().byteSize() * index);
    }

    /**
     * The size (in bytes) of this struct
     */
    public static long sizeof() {
        return layout().byteSize();
    }

    /**
     * Allocate a segment of size {@code layout().byteSize()} using
     * {@code allocator}
     */
    public static MemorySegment allocate(SegmentAllocator allocator) {
        return allocator.allocate(layout());
    }

    /**
     * Allocate an array of size {@code elementCount} using {@code allocator}.
     * The returned segment has size {@code elementCount * layout().byteSize()}.
     */
    public static MemorySegment allocateArray(long elementCount, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(elementCount, layout()));
    }

    /**
     * Reinterprets {@code addr} using target {@code arena} and
     * {@code cleanupAction} (if any).
     * The returned segment has size {@code layout().byteSize()}
     */
    public static MemorySegment reinterpret(MemorySegment addr, Arena arena, Consumer<MemorySegment> cleanup) {
        return reinterpret(addr, 1, arena, cleanup);
    }

    /**
     * Reinterprets {@code addr} using target {@code arena} and
     * {@code cleanupAction} (if any).
     * The returned segment has size {@code elementCount * layout().byteSize()}
     */
    public static MemorySegment reinterpret(MemorySegment addr, long elementCount, Arena arena,
            Consumer<MemorySegment> cleanup) {
        return addr.reinterpret(layout().byteSize() * elementCount, arena, cleanup);
    }
}
